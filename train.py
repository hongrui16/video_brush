...
from network.pipelines.pipeline_vbrush import VBrushPipeline
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if use_vbrush:
        if not image_finetune:
            vbrushnet = VBrushUNet3DConditionModel()
    else:
        vbrushnet = None
    ...
    train_dataloader, train_dataset = build_dataloader("WebVid10M", **config)
    ...
    # Validation pipeline
    if not image_finetune:
        validation_pipeline = VBrushPipeline(
            unet=unet, vae=vae, tokenizer=tokenizer, text_encoder=text_encoder, scheduler=noise_scheduler, vbrushnet=vbrushnet,
        ).to("cuda")
    else:
