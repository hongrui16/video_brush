...
from diffusers.models.unet_2d_condition import UNet2DConditionModel, CrossAttnDownBlock2D, DownBlock2D

from network.models.unet_blocks import (
...
from network.models.resnet import InflatedConv3d, InflatedGroupNorm
...
@dataclass
class VBrushUNet3DConditionOutput(BaseOutput):
    down_block_res_samples: Tuple[torch.Tensor]
    mid_block_res_sample: torch.Tensor
    up_block_res_samples: Tuple[torch.Tensor]


class VBrushUNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if use_simplified_condition_embedding:
            ...
            self.controlnet_cond_embedding = ConditioningEmbedding(
    ...
    def forward(
    ...
    ) -> Union[VBrushUNet3DConditionOutput, Tuple]:
        ...
        for up_block_res_sample, controlnet_up_block in zip(up_block_res_samples, self.controlnet_up_blocks):
            up_block_res_sample = controlnet_up_block(up_block_res_sample)
        ...
        if guess_mode and not self.config.global_pool_conditions:
            scales = torch.logspace(-1, 0, len(down_block_res_samples) + 1 + len(up_block_res_sample), device=sample.device)  # 0.1 to 1.0
            scales = scales * conditioning_scale
            down_block_res_samples = [sample * scale for sample, scale in zip(down_block_res_samples, scales[:len(down_block_res_samples)])]
            mid_block_res_sample = mid_block_res_sample * scales[len(down_block_res_samples)]  # last one
            up_block_res_samples = [sample * scale for sample, scale in zip(up_block_res_samples, scales[len(down_block_res_samples) + 1:])]
        else:
        ...
        return VBrushUNet3DConditionOutput(
